...
export default function Session() {
  ...
  // Start camera + WebSocket once user hits Start
  useEffect(() => {
  ...
  // Pre-start screen — shown until user clicks Start
  if (!started) {
    return (
      <div className="session-page session-page--ready">
        <div className="ready-card">
          <span className="ready-group">{exercise.group}</span>
          <h1 className="ready-name">{exercise.name}</h1>
          {exercise.repDetection ? (
            <p className="ready-hint">Camera + pose detection will track your reps automatically.</p>
          ) : (
            <p className="ready-hint">This is a hold / time-based exercise — camera will show your form.</p>
          )}
          {!modelReady && (
            <p className="ready-loading">Loading pose model…</p>
          )}
          <button
            ...
            disabled={!modelReady}
          >
            {modelReady ? 'Start Exercise' : 'Loading…'}
          </button>
          <button className="ready-back-btn" onClick={() => navigate(`/exercises/${id}`)}>
            Back
          </button>
        </div>
      </div>
